    ...
    with gr.Blocks(analytics_enabled=False) as sd_image_info:
        with FormRow(equal_height=False):
            with FormColumn(variant="compact", scale=3):
                image = gr.Image(elem_id="imgInfoImage", source="upload", interactive=True, type="pil", show_label=False)
            ...
            with FormColumn(variant="compact", scale=7, elem_id="imgInfoOutputPanel"):
                geninfo = gr.Textbox(elem_id="imgInfoGenInfo", label="RAW", visible=False)
        ...
        image.change(fn=None, inputs=[], outputs=[], _js="() => {image_info_parser();}")
